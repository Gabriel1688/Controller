...
Planning or trajectory optimization is searching for {u[⋅],x[⋅]} (with w[⋅]=0). Robust planning includes taking an expectation or worst case over w.
State estimation is searching for {x[⋅],w[⋅]}
System identification is searching for {p, w[⋅], and often x[⋅]}
Stability analysis is (for w=0), for instance, finding a set of initial conditions x[0] for which lim n→ ∞, x[n] → 0. Stochastic stability analysis asks, e.g. the probability of leaving a region over some finite-time horizon.
